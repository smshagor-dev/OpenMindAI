...
class AttachmentContextService {
  ...
  Future<String> _extractPdf(String path) async {
    final file = File(path);
    if (!await file.exists()) return 'The selected PDF is no longer available.';
    try {
      final result = await PdfStructuredExtractor.extractFromFile(path);
      final pages = result['pages'];
      if (pages is! List) return 'No readable PDF text was found.';
          ...
          if (type == 'table') {
            ...
            if (rows is List) {
              for (final row in rows) {
                if (row is List) output.writeln(row.map((cell) => cell.toString()).join(' | '));
              }
            }
          } else {
            final value = block['text']?.toString().trim();
            if (value != null && value.isNotEmpty) output.writeln(value);
          }
      ...
      return output.isEmpty ? 'No readable PDF text was found.' : output.toString();
    } catch (_) {
