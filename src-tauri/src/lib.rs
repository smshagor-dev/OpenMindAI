...
fn resolve_conversation_model(
    ...
    let selected =
        select_conversation_model(&models, active_model_id.as_deref()).ok_or_else(|| {
            app_error::AppError::ModelNotFound(
                "Qwen3 4B Q4_K_M is not installed. Download it from Settings > Models first."
                    .to_string(),
            )
        })?;
...
async fn send_chat_message(
    ...
    let model = resolve_conversation_model(&state, &conversation_id)?;
            ...
            models
                ...
                .find(|model| {
                    model
                        .source_repository
                        .as_deref()
                        .is_some_and(|repo| repo == "Qwen/Qwen3-4B-GGUF")
                })
                .cloned()
        })
...
mod milestone2_tests {
    ...
    fn select_conversation_model_prefers_active_binding() {
        ...
        let selected = select_conversation_model(&models, Some("other")).unwrap();
    ...
    fn select_conversation_model_falls_back_to_qwen_then_enabled() {
        ...
        let selected = select_conversation_model(&models, None).unwrap();
        assert_eq!(selected.id, "qwen");

        let models_without_qwen = vec![other.clone()];
        let fallback = select_conversation_model(&models_without_qwen, Some("missing")).unwrap();
        assert_eq!(fallback.id, "other");

        let no_models: Vec<ModelRecord> = Vec::new();
        assert!(select_conversation_model(&no_models, None).is_none());
    }
}
