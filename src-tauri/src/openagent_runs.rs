...
impl<'a> OpenAgentRunRepository<'a> {
    ...
    pub fn start(
        ...
        self.find(&id)?
            .ok_or_else(|| AppError::internal("new OpenAgent run was not found"))
    }
    ...
    pub fn details(&self, run_id: &str) -> Result<Option<OpenAgentRunDetails>, AppError> {
        let Some(run) = self.find(run_id)? else {
            return Ok(None);
        };
        let mut statement = self.database.connection().prepare(
...
fn map_step(row: &Row<'_>) -> rusqlite::Result<OpenAgentStep> {
    Ok(OpenAgentStep {
        ...
        status: row.get(5)?,
        workspace_changed: row.get::<_, i64>(6)? != 0,
        validation_command: row.get(7)?,
...
#[tauri::command]
pub fn list_openagent_runs(
    conversation_id: String,
    limit: Option<usize>,
    state: State<AppState>,
) -> Result<Vec<OpenAgentRun>, AppError> {
    let db = state
        .database
        .lock()
...
#[tauri::command]
pub fn openagent_run_details(
    run_id: String,
    state: State<AppState>,
) -> Result<Option<OpenAgentRunDetails>, AppError> {
    let db = state
        .database
        .lock()
        .map_err(|_| AppError::internal("database lock poisoned"))?;
